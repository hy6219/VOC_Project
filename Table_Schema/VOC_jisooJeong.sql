...
CREATE TABLE `CS` (
	...
	`EPART`	INT	NULL COMMENT '담당 클레임 분류',
	`ENAME`	VARCHAR(2000)	NULL COMMENT '직원 이름',
	`EMAIL`	VARCHAR(2000)	NULL COMMENT '직원 이메일',
	`AFFORD`	VARCHAR(100)	NULL COMMENT '클레임 담당 가능 여부',
...
CREATE TABLE `REIM` (
	`REIM_ID`	BIGINT	NOT NULL COMMENT 'REIM ID',
	`REIM_DATE`	TIMESTAMP	NULL COMMENT '배상 처리 일자',
	`VOC_ID`	BIGINT	NOT NULL COMMENT 'VOC ID'
);
